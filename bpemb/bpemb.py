...
class BPEmb():
    ...
    """
    emb_tpl_vec = "{lang}/{lang}.bpe.vs{vs}.d{dim}.vec"
    emb_tpl_bin = "{lang}/{lang}.bpe.vs{vs}.d{dim}.bin"
    model_tpl = "{lang}/{lang}.bpe.vs{vs}.model"
    ...
    def __init__(
        ...
        self.model_file = Path(cache_dir) / model_file
        print(f"Loading local BPE model: {self.model_file}")
        self.spm = sentencepiece_load(self.model_file)
        if encode_extra_options:
            self.spm.SetEncodeExtraOptions(encode_extra_options)
        emb_file = self.emb_tpl_vec.format(lang=lang, vs=vs, dim=dim)
        self.emb_file = Path(cache_dir) / emb_file
        if not self.emb_file.exists():
            emb_file = self.emb_tpl_bin.format(lang=lang, vs=vs, dim=dim)
            self.emb_file = Path(cache_dir) / emb_file
        if not self.emb_file.exists():
